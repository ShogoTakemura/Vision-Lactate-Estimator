...
def process(config: ConfigParser):
    ...
    # ユーザーに, correctpickleか, modelbasedpickleかを尋ねる.
    ...
    select = selection(correct_pickle_paths)
    pathobj = correct_pickle_paths[select]

    with open(pathobj.resolve(), 'rb') as f:
        loadmove3d = pickle.load(f)

    # TODO #42 多視点動画の作成処理の追加
    writer3d = posewriter.Video3DWriter(
        loadmove3d, f"{pathobj.stem}_side.mp4", outdir, elev_angle=0, yaw_angle=0)
    ...
    writer3d = posewriter.Video3DWriter(
        loadmove3d, f"{pathobj.stem}_front.mp4", outdir, elev_angle=0, yaw_angle=90)
    writer3d.videosetting(config.getint('movie', 'width'), config.getint(
        'movie', 'height'), config.getfloat('movie', 'fps'))
    writer3d.write3dpose()
    writer3d.release()
    ...
    writer3d = posewriter.Video3DWriter(
        loadmove3d, f"{pathobj.stem}_top.mp4", outdir, elev_angle=90, yaw_angle=0)
    writer3d.videosetting(config.getint('movie', 'width'), config.getint(
        'movie', 'height'), config.getfloat('movie', 'fps'))
    writer3d.write3dpose()
    writer3d.release()
